 ...
 */
@Controller
public class BindCarAndTelController extends ControllerBaseByLogin  {
	...
	@RequestMapping(value = "bindcarAccount.get")
	@Override
	...
	protected void doService(RequestParam requestParam,	JsonGenerator responseBodyJson, HttpServletRequest request)	throws ShopException, JsonGenerationException, IOException  {
		...
		{
			for(TerminalInfo a:terminalList)
			{
				int status=a.getTerminal_status();
				if(status==1)
				{
			...
			{
				logger.info("bindcarAccount.get 往terminal表中插入数据失败");
			...
			if(accid !=null)
			{
				logger.info("bindcarAccount.get 往account表中已经存在此terminalimei信息");
			}else
			{
				//创建account对象
				...
				{
					logger.info("bindcarAccount.get 往account表中插入数据失败");
					//responseBodyJson.writeStringField("result", "0");
			...
			//根据terminal_imei查询user_id
				...
				if(null != terList && terList.size()>0)
				{
					...
					{
						logger.info("bindcarAccount.get 往user表中插入数据失败");
						//responseBodyJson.writeStringField("result", "0");
			...
			{
				logger.info("bindcarAccount.get 往person表中插入数据失败");
				//responseBodyJson.writeStringField("result", "0");
			...
			List<TerminalInfo> terminalListNew = terminalInfoService.selectUserIdByImei(imei);
			...
			if (accountList == null || accountList.size() <= 0) {
				...
				for (TerminalInfo ta : terminalListNew) {
					UsUser usUser = new UsUser();
					...
					TerminalInfo terminalInfo = new TerminalInfo();
					terminalInfo.setUser_id(ta.getUser_id());
				...
				int user_id = 0;
				user_id = terminalListNew.get(0).getUser_id(); // 得到userid
				//int car_id = carInfoService.getCarId("car_id");
			    //////集群 20150408
				int value2 = carInfoService.queryId("car_id")+1;  //查询数据库序列值
				int car_id = value2;
				TestUpd testupd2 = new TestUpd();
				testupd2.setName("car_id");
				...
				try {
					...
					logger.info("bindcarAccount.get 往terminal_car表中插入数据失败");
					Object[] params = new Object[1];
					params[0] = "用户车辆信息";
					throw new ShopBusiException(ShopBusiErrorBundle.INSERT_FAILURE, params);
				}
