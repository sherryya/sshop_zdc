 ...
 */
@Controller
public class UpdItovShop4sController extends ControllerBase {
	Logger logger = Logger.getLogger(Getlistbypage.class);
	@Autowired
	private TItov_shop4sService tItov_shop4sService;

	@RequestMapping(value = "updateShop4sinfo.upd")
	@Override
	...
	protected void doService(RequestParam requestParam, JsonGenerator responseBodyJson) throws ShopException, JsonGenerationException, IOException {
		...
		int id = ParamData.getInt(requestParam.getBodyNode(), "shop4s_id");
		...
		tItov_shop4s_manage.setShop4s_id(id);
		...
		try{
			...
			int count = tItov_shop4sService.updateShop4sId(tItov_shop4s_manage);
