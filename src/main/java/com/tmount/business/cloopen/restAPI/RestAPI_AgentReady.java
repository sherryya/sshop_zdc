...
public class RestAPI_AgentReady {
    /**
     ...
     */
	public  String getAgentReady( String agentid) throws NoSuchAlgorithmException, KeyManagementException {
		...
		try {
			...
			String url = (new StringBuilder(hostname)).append(":").append(port)
					     ...
					     .append("/ivr/agentready?sig=").append(signature).append("&agentid=").append(agentid).toString();
			...
			if(!JsonUtils.JSON_ENABLE) {
				...
				body = (new StringBuilder("<?xml version='1.0' encoding='utf-8'?><Request>")
	              ...
	              .append("<AgentReady  ").append("agentid=\"").append(agentid).append("\"").append("/>")
