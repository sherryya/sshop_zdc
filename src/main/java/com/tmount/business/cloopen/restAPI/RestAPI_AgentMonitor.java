...
public class RestAPI_AgentMonitor {
	public  String GetAgentCallForwarding( String agentid,String callid,String number) throws NoSuchAlgorithmException, KeyManagementException {
		...
		try {
			...
			if(!JsonUtils.JSON_ENABLE) {
				...
				body = (new StringBuilder("<?xml version='1.0' encoding='utf-8'?><Request>"))
			            ...
			            .append("<Monitor").append("  agentid=\"").append(agentid).append("\" number=\""+number+"\" callid=\""+callid+"\"").append(">")
			            .append("</Monitor></Request>").toString();
			} else {
				AgentReady agentReady= new AgentReady(appId, agentid);
				Gson gson = new Gson();
				body = gson.toJson(agentReady);
			}
			System.out.println("Monitor : request _" + body);
	...
	public static void main(String[] args) throws KeyManagementException, NoSuchAlgorithmException {
		RestAPI_AgentMonitor r=new  RestAPI_AgentMonitor();
		r.GetAgentCallForwarding("1000", "14061317004989780001000500005720", "18646117093");
	}
}
