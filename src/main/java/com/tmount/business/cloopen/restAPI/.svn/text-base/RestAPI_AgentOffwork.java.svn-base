...
import com.tmount.business.cloopen.model.httprequest.AgentOffwork;
...
public class RestAPI_AgentOffwork {
	// 请求的主机名（IP或者域名）,端口号

    /**
     * @param number 座席号码，手机号或座机号或voip帐号
     * @param agentid 座席ID，4位正整数，由应用侧管理
     * @return
     * @throws NoSuchAlgorithmException
     * @throws KeyManagementException
     */
	public  String getAgentOffWork(String number, String agentid) throws NoSuchAlgorithmException, KeyManagementException {
		...
		try {
			...
			String url = (new StringBuilder(hostname)).append(":").append(port)
					     ...
					     .append("/ivr/agentoffwork?sig=").append(signature).append("&number=").append(number).append("&agentid=").append(agentid).toString();
			...
			if(!JsonUtils.JSON_ENABLE) {
				...
				body = (new StringBuilder("<?xml version='1.0' encoding='utf-8'?><Request>")
	              ...
	              .append("<AgentOffWork  ").append("agentid=\"").append(agentid).append("\"").append("/>")
	              .append("</Request>").toString();
				System.out.println("body:"+body);
			} else {
				AgentOffwork agentOffwork= new AgentOffwork(appId, number, agentid);
				Gson gson = new Gson();
				body = gson.toJson(agentOffwork);
			}
