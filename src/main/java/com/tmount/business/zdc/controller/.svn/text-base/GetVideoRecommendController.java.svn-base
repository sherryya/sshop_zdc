 ...
 */
@Controller
public class GetVideoRecommendController extends ControllerBaseByLogin {
	Logger logger = Logger.getLogger(GetVideoRecommendController.class.getName());
	@RequestMapping(value = "VideoRecommendController.get")
	@Override
	...
	protected void doService(RequestParam requestParam, JsonGenerator responseBodyJson, HttpServletRequest request) throws ShopException, JsonGenerationException, IOException {
		...
		String url=domain_65Net+"recommend.json?mid="+mid+"&num="+num+"&page="+page+"&sign="+sign+"&appkey=1000000010&ts="+datetimestamp+"";
